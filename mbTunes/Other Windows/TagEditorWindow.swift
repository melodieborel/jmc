...
class TagEditorWindow: NSWindowController, NSTextFieldDelegate, NSWindowDelegate {
    ...
    func populateArtwork() {
        guard selectedTracks![0].album != nil else { return }
        ...
        let newConstraint = NSLayoutConstraint(item: self.albumFilesViewController?.otherArtBox! as Any, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: 90.0)
